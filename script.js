...
const carData = [
  {year:'2024',season:'Current Build - Season 9',name:'TOR-24',image:'assets/tor-24.jpg',desc:'Our most refined machine to date with a laser-cut spaceframe, full aero package and improved high-voltage systems for Formula Student Germany 2024.',specs:{'Peak Power':'80 kW','0-100 km/h':'3.6 s','Accumulator':'6.2 kWh','Mass':'238 kg','Downforce':'420 N','Wheelbase':'1540 mm'}},
  {year:'2020',season:'Season 8',name:'TOR-20',image:'assets/tor-20-render.png',desc:'A major step forward with full aero development, improved powertrain packaging and stronger validation workflows.',specs:{'Peak Power':'72 kW','0-100 km/h':'4.8 s','Accumulator':'5.8 kWh','Mass':'252 kg'}},
  {year:'2018',season:'Season 7',name:'TOR-18',image:'assets/tor-18.webp',desc:'A pre-pandemic benchmark generation focused on manufacturing maturity, race reliability and stronger static event preparation.',specs:{'Peak Power':'60 kW','0-100 km/h':'5.6 s','Accumulator':'5.0 kWh','Mass':'268 kg'}},
  {year:'2017',season:'Season 6',name:'TOR-17',image:'assets/tor-17-render.png',desc:'An important design iteration that helped mature the team architecture, testing discipline and subsystem ownership.',specs:{'Peak Power':'60 kW','0-100 km/h':'5.6 s','Accumulator':'5.0 kWh','Mass':'268 kg'}},
  {year:'2016',season:'Season 5',name:'TOR-16',image:'assets/tor-16.png',desc:'A foundational competitive car that strengthened the team culture and set the direction for future electric race builds.',specs:{'Peak Power':'55 kW','0-100 km/h':'5.9 s','Accumulator':'4.8 kWh','Mass':'272 kg'}},
  {year:'2012-15',season:'Origin Years',name:'Vidhyut to TOR-15',image:'assets/tor-18-render.png',desc:'The years where Team Ojas built its first cars, learned by doing and created the engineering culture that still powers the team.',specs:{'Generations':'4','Focus':'Learning','Platform':'Electric','Legacy':'Foundation'}}
];
...
function renderCars(activeYear = carData[0].year){
  ...
  const car = carData.find(item => item.year === activeYear) || carData[0];
  display.innerHTML = `<article class="car-info"><span class="car-badge">${car.season}</span><h2 class="car-name">${formatCarName(car.name)}</h2><p>${car.desc}</p><div class="spec-grid">${Object.entries(car.specs).map(([k,v]) => `<div><span>${k}</span><b>${v}</b></div>`).join('')}</div></article><figure class="car-visual"><span class="callout">${car.name} - ${car.year}</span><img src="${car.image}" alt="${car.name}"></figure>`;
  rail.querySelectorAll('button').forEach(button => button.addEventListener('click', () => renderCars(button.dataset.year)));
}
